...
vhost_skeleton_path="$(echo $HOME)/scripts/vhost.skeleton.conf"
web_root="/var/www/"
...
# construct absolute path
absolute_doc_root=$relative_doc_root
fi

absolute_doc_root=$(readlink -f $absolute_doc_root)
...
if [ ! -d "$absolute_doc_root" ]; then
  if [ $is_new == 1 ]; then   
    ...
    echo "website folder doesn't exists !"
    exit 2
  fi
fi

echo "$absolute_doc_root successfully linked !"
# update vhost
